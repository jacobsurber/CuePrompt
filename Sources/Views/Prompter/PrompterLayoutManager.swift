...
final class PrompterLayoutManager: NSLayoutManager {
    ...
    static let highlightColor = CueColors.accentNS.withAlphaComponent(0.25)
    ...
    override func fillBackgroundRectArray(
        ...
        for i in 0..<rectCount {
            ...
            let glowPath = CGPath(
                roundedRect: glowRect, cornerWidth: glowRadius, cornerHeight: glowRadius,
                transform: nil)
            context.addPath(glowPath)
            context.setFillColor(CueColors.accentNS.withAlphaComponent(0.08).cgColor)
            context.fillPath()
            ...
            let mainPath = CGPath(
                roundedRect: mainRect, cornerWidth: mainRadius, cornerHeight: mainRadius,
                transform: nil)
    ...
    private func isHighlightColor(_ color: NSColor) -> Bool {
        ...
        // Our highlight is amber (~0.91, 0.64, 0.19) at 0.25 alpha
        let isAmber =
            c.redComponent > 0.8 && c.greenComponent > 0.5 && c.greenComponent < 0.8
            && c.blueComponent < 0.3
            && c.alphaComponent > 0.15 && c.alphaComponent < 0.35
        return isAmber
    }
}
